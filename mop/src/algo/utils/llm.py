...
from algo.utils.models import Llama2Wrapper
...
async def dispatch_openai_requests(
    messages_list: List[List[Dict[AnyStr,Any]]],
    model: str,
    temperature: float,
    max_tokens: int,
    ...
    # for x in messages_list:
        # try:
    async_responses = [client.ChatCompletion.acreate(
            model=model,
            messages=x,
...
class Llama_Forward(LLM):
    ...
    def __init__(self, config, needs_confirmation=False, disable_tqdm=True):
        ...
        self.config = config
        self.device = config['evaluation']['model']['device']
        self.needs_confirmation = needs_confirmation
        self.disable_tqdm = disable_tqdm
        
        llama_dir = 'hf_cache/meta-llama/'

        ## TODO under dev ##
        if 'llama_config' in config and config['llama_config']['model'] == 'llama2-7b-chat':
            logging.info('loading llama2-7b-chat')
            MODEL_DIR = os.path.join(llama_dir, "llama2-hf/7B-chat")
            self.model = Llama2Wrapper(MODEL_DIR, self.device)
        else:
            logging.info('loading default llama')
            MODEL_DIR = os.path.join(llama_dir, 'llama-hf/7B')
    ...
    def generate_text(self, prompts, n):
        ...
        for prompt in prompts:
            for _ in range(n):
                if isinstance(self.model, Llama2Wrapper):
                    text.append(self.model.generate(prompt, max_new_tokens=32))
                else:
...
class GPT_Forward(LLM):
    ...
    def __generate_text(self, prompt, n):
        ...
        response, ret = None, None
        while ret is None:
            try:
                if config['model'] == 'gpt-3.5-turbo':
                    assert False, 'library deprecated'
                    response = []
                    ...
                    ret = [response[i]['message']['content'] for i in range(len(response))]
                else:
                    responses = client.completions.create(
                        **config,
                        prompt=prompt).choices #[0].text.lstrip()
                    ret = [response.text.lstrip() for response in responses]
