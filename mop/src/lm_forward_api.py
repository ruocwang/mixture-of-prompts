...
from algo.utils import evaluate, config, template
import re

from algo.instruct_zero.instruction_coupled_kernel import *
...
class LMForwardAPI:
    def __init__(self, model_name='vicuna', eval_data=None, init_prompt=None, init_qa=None, conf=None, base_conf=None,
        ...
        if api_model in ['llama', 'flan-t5']:
            self.api_model = evaluate.exec_evaluator(api_model, self.conf)
    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        input_ids = self.tokenizer(self.init_token, return_tensors="pt").input_ids.to(self.device)
        ...
        if instruction[0] in self.prompts_set.keys():  ## visited
            ...
            if api_model in ['chatgpt']: 
                dev_perf, instruction_score, _ = evaluate.evaluate_prompts(instruction, self.eval_template, self.eval_data, self.demos_template, self.few_shot_data, self.conf['evaluation']['method'], self.conf['evaluation'])
